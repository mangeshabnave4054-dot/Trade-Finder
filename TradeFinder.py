...
if strategy_choice == "Strategy 1: EMA Crossover & RS Trend":
    ...
    if run_scan_1:
        # 1. Download & calculate inside the spinner
        with st.spinner("Checking NIFTY 50 200 EMA Regime..."):
            nifty_daily = yf.download("^NSEI", period="2y", interval="1d", progress=False)
        ...
        # 2. Check and stop OUTSIDE the spinner (un-indented by 4 spaces)
        if latest_nifty_close < latest_nifty_ema200:
            ...
            # st.stop()  # Halts execution cleanly AFTER the spinner has closed
        else:
        ...
        nifty_weekly = yf.download("^NSEI", period="3y", interval="1wk", progress=False)
        ...
        for i, sym in enumerate(symbols):
            ...
            try:
                df_w = yf.download(sym, period="3y", interval="1wk", progress=False)
                if df_w.empty or len(df_w) < 60: continue
                ...
                if not (bullish_crossover and rsi_condition): continue
                ...
                df_d = yf.download(sym, period="6mo", interval="1d", progress=False)
                if isinstance(df_d.columns, pd.MultiIndex): df_d.columns = df_d.columns.get_level_values(0)
                
                curr_price = float(df_d['Close'].iloc[-1])
                swing_low = float(df_d['Low'].iloc[-10:].min())
                risk = curr_price - swing_low
    ...
    if 'scan_results_1' in st.session_state:
        ...
        if selected_stock:
            ...
            interval = "1wk" if "Weekly" in chart_tf else "1d"
            
            stock_w = yf.download(f"{selected_stock}.NS", period="2y", interval=interval, progress=False)
            if isinstance(stock_w.columns, pd.MultiIndex): stock_w.columns = stock_w.columns.get_level_values(0)
                
            stock_w['EMA21'] = ta.ema(stock_w['Close'], length=21)
            stock_w['EMA55'] = ta.ema(stock_w['Close'], length=55)
            stock_w['RSI14'] = ta.rsi(stock_w['Close'], length=14)
            
            bench_df = yf.download("^NSEI", period="2y", interval=interval, progress=False)
            if isinstance(bench_df.columns, pd.MultiIndex): bench_df.columns = bench_df.columns.get_level_values(0)

            common_idx = stock_w.index.intersection(bench_df.index)
            rs = calculate_custom_rs(stock_w.loc[common_idx, 'Close'], bench_df.loc[common_idx, 'Close'], rs_length)
            x_pos, y_pos, x_neg, y_neg = split_series_at_zero(rs.dropna())

            # --- Date Window & Padding Setup ---
            total_bars = len(stock_w)
            view_window = 180
            start_date = stock_w.index[-view_window] if total_bars > view_window else stock_w.index[0]
            end_date = stock_w.index[-1]
            end_date_padded = end_date + pd.Timedelta(days=10)

            # Update Subplots to include Titles
            fig = make_subplots(
                rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.5, 0.25, 0.25],
                subplot_titles=(
                    f"{selected_stock} Price & EMAs", 
                    "Relative Strength (vs Nifty 50)", 
                    "Relative Strength Index (14)"
                )
            )
            
            # --- Row 1: Candlesticks and EMAs ---
            fig.add_trace(go.Candlestick(x=stock_w.index, open=stock_w['Open'], high=stock_w['High'], low=stock_w['Low'], close=stock_w['Close'], name='Price'), row=1, col=1)
            fig.add_trace(go.Scatter(x=stock_w.index, y=stock_w['EMA21'], line=dict(color='blue', width=1.5), name='21 EMA'), row=1, col=1)
            fig.add_trace(go.Scatter(x=stock_w.index, y=stock_w['EMA55'], line=dict(color='orange', width=1.5), name='55 EMA'), row=1, col=1)
            
            # --- Row 2: Relative Strength (Green > 0, Red < 0) ---
            ...
            # Bulletproof Reference Lines & Shading for Row 3
            fig.add_trace(go.Scatter(x=[stock_w.index[0], end_date_padded], y=[70, 70], mode="lines", line=dict(color="#0037FF", dash="dash", width=1), hoverinfo="skip"), row=3, col=1)
            
            fig.add_trace(go.Scatter(
                x=[stock_w.index[0], end_date_padded], y=[30, 30], 
                mode="lines", line=dict(color="#0037FF", dash="dash", width=1), 
                fill="tonexty", fillcolor="rgba(126, 87, 194, 0.12)", hoverinfo="skip"
            ), row=3, col=1)
            
            fig.add_trace(go.Scatter(x=[stock_w.index[0], end_date_padded], y=[50, 50], mode="lines", line=dict(color="rgba(120, 123, 134, 0.5)", dash="dot", width=1), hoverinfo="skip"), row=3, col=1)
            ...
            fig.update_xaxes(
                ...
                range=[start_date, end_date_padded]
            )
            ...
            # Fix scales to 0-100 for the RSI indicator
...
# =====================================================================
    ...
    if run_scan_2:
        ...
        for i, sym in enumerate(symbols):
            progress_bar.progress((i + 1) / len(symbols))
            status_text.text(f"Scanning {sym} ({i+1}/{len(symbols)})...")
    ...
    if 'scan_results_2' in st.session_state:
        ...
        if selected_stock:
            ...
            fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.54, 0.23, 0.23])
            ...
            fig.add_hrect(y0=30, y1=70, fillcolor="rgba(126, 87, 194, 0.12)", line_width=0, layer="below", row=3, col=1)
            
            fig.add_hline(y=70, line_dash="dash", line_color="#0037FF", row=3, col=1)
            fig.add_hline(y=50, line_dash="dot", line_color="rgba(120, 123, 134, 0.5)", row=3, col=1)
            fig.add_hline(y=30, line_dash="dash", line_color="#0037FF", row=3, col=1)
